...
function LivePreview({ code, subjectType, demoHtml }) {
  ...
  let doc
  if (subjectType === 'css') {
    // Use concept-specific demoHtml if provided, else fall back to generic scaffold
...
${code}
    ...
    // demoHtml provides interactive HTML elements (for DOM/Events concepts)
    // Without demoHtml: just show console output in #output div
...
${code}
    ...
    // Detect external package imports that can't run in browser without npm
...
<div style="text-align:center;padding:16px;max-width:300px;">
  ...
  <div style="font-size:11px;color:#9CA3AF;margin-top:8px;">Study the code above — run it in a real React project</div>
    ...
    // Transform ES module imports → CDN globals so React examples work without bundler
    let cleanCode = code
    ...
    // Auto-inject React.hook declarations for hooks used without prefix or import
    // Handles examples that use useState/useRef/useEffect without importing them
    ...
    // Detect the root component — PascalCase definition that is NOT used as a JSX child
    ...
    // Root = last defined component that is never used as <Child/> inside other components
...
<\/script>
    ...
    } // end else (no external packages)
  } else {
    // HTML — use as body content if no <html>, else as-is
...
function ConceptInlinePanel({ conceptId, navList, onClose, navigate, startQuiz, subjectTitle = '' }) {
